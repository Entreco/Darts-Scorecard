...
class Play01ViewModel @Inject constructor(private val playGameUsecase: Play01Usecase, private val gameListeners: Play01Listeners, private val logger: Logger) : BaseViewModel(), UiCallback, InputListener {
    ...
    fun load(request: Play01Request, load: GameLoadedNotifier<CreateGameRequest>, vararg loaders: GameLoadedNotifier<List<Pair<Long, Turn>>>) {
        playGameUsecase.loadGameAndStart(request,
                { response ->
                    ...
                    loaders.forEach {
                        it.onLoaded(response.teams, game.scores, response.turns, null)
                    }
                },
    ...
    fun registerListeners(scoreListener: ScoreListener, specialEventListener: SpecialEventListener<*>, statListener: MatchStatListener, vararg playerListeners: PlayerListener){
        gameListeners.registerListeners(scoreListener, specialEventListener, statListener, *playerListeners)
    }
    ...
    private fun storeTurn(turn: Turn, by: Player) {
        ...
        playGameUsecase.storeTurnAndStats(turnRequest, stats)
    }
