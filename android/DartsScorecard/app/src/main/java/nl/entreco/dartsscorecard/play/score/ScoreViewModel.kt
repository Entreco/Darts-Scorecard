 ...
 */
open class ScoreViewModel @Inject constructor(val teams: Array<Team>, val scoreSettings: ScoreSettings, val adapter: ScoreAdapter) : BaseViewModel(), ScoreListener, PlayerListener {

    val numSets = ObservableInt(scoreSettings.numSets)

    override fun onScoreChange(scores: Array<Score>, by: Player) {
        Log.d("NoNICE", "1:$scores by:$by")
        scores.forEachIndexed { index, score -> adapter.teamAtIndexScored(index, score, by); }
    }

    override fun onDartThrown(turn: Turn, by: Player) {
        Log.d("NoNICE", "onDartThrown: ${turn.last()} from:$by")
