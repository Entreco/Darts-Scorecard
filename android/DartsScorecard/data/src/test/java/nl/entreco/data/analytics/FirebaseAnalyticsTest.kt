...
class FirebaseAnalyticsTest {
    ...
    fun `it should parse price with ','`() {
        assertEquals(7.99, subject.formatMicros(7990000L), 0.01)
    }

    @Test
    fun `it should parse price from SKU ()`() {
        assertEquals(0.0, subject.formatMicros(59L), 0.01)
    }
    ...
    fun `it should have correct Donation bundle`() {
        val bundle = subject.toBundle(Donation("title", "desc", "sku", "12", 100, "EUR", 1000000L))
        assertNotNull(bundle)
    }
